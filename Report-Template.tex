...
\begin{algorithmic}
	\State Input: set[ ], n
	\State Initialise sb[n+1][len(set)+1]
	\State Assign all sb[0][ ] = True;
	\State Assign all except first: sb[ ][0] = False;
	\For{$i=1$ to $ n $}
	\For{$j=1$ to $ len(set)$}
	\State sb[i][j] = sb[i][j-1]
	\If{(i greate or equal than set[j-1])}
	\State sb[i][j] = sb[i][j] OR sb[i  -  set[j-1]][j-1]
	\EndIf
	\EndFor
	\EndFor
	\If{(sb[n][len(set)])}
